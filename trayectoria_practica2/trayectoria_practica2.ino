...
#define FIXED_PLAYBACK_SPEED_L 145 // Velocidad fija (de 0 a 255) //

// Debounce
#define DEBOUNCE_DELAY 1000 // 1 segundo anti-rebote para pulsadores
#define ENCODER_DEBOUNCE_MS 50 // Debounce para encoders

//velocidad mínima (para que el robot se mueva)
#define MIN_PLAYBACK_SPEED 100
// Definan el máximo de ticks que esperan en un segmento (requiere experimentación)
#define MAX_TICKS_PER_SEGMENT 200 
...
// Función de ejecución PROPORCIONAL
void executeSegmentProportional(int index)
{
  ...
  // Si no hay movimiento grabado (ticks = 0), solo esperamos
  ...
  // Mapear los ticks a una velocidad
  int speedLeft = map(targetLeft, 1, MAX_TICKS_PER_SEGMENT, MIN_PLAYBACK_SPEED, 255);
  int speedRight = map(targetRight, 1, MAX_TICKS_PER_SEGMENT, MIN_PLAYBACK_SPEED, 255);

  // Asegurarse de que no se pase de los límites (constrain)
  speedLeft = constrain(speedLeft, 0, 255);
  speedRight = constrain(speedRight, 0, 255);

  // Si solo gira una rueda, la otra debe estar en 0
  if (targetLeft <= 0) speedLeft = 0;
  if (targetRight <= 0) speedRight = 0;
  
  Serial.print("Segmento "); Serial.print(index);
  Serial.print(": Ticks(L,R)= "); Serial.print(targetLeft); Serial.print(","); Serial.print(targetRight);
  Serial.print("  Vel(L,R)= "); Serial.print(speedLeft); Serial.print(","); Serial.println(speedRight);

  // Aplicamos la velocidad y esperamos
...
void handlePlaying()
{
  ...
  {
    Serial.print("Ejecutando segmento "); Serial.println(i);
    // ¡CAMBIO AQUÍ! Usar la función PROPORCIONAL
    executeSegmentProportional(i); 
  }
